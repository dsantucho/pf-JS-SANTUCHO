...
function getLocalStorage(value, key){value = JSON.parse(localStorage.getItem(key))};
...
function addItemSupermarket() {
    ...
    console.log(`ADD NewItem ==>\n producto:${newItem.item}; precio: ${newItem.price}`); //muestra todo la info
    return newItem;
}
...
function listarItems(arr){
    ...
    for(let index = 0; index < keys.length; index++){
        if(arr[index].active){
            console.log(`Item nro ${index}: es ${arr[index].item}; Precio: ${arr[index].price} `);
        }      
    }
}
